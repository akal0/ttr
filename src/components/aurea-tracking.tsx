"use client";

import { useEffect } from "react";
import { initAurea } from "aurea-tracking-sdk";

export function AureaTracking() {
  useEffect(() => {
    // Only initialize if we have the required env vars
    const apiKey = process.env.NEXT_PUBLIC_AUREA_API_KEY;
    const funnelId = process.env.NEXT_PUBLIC_AUREA_FUNNEL_ID;
    const apiUrl = process.env.NEXT_PUBLIC_AUREA_API_URL;

    // Debug: Log environment variables
    console.log("[Aurea] Configuration:", {
      hasApiKey: !!apiKey,
      hasFunnelId: !!funnelId,
      apiUrl: apiUrl || "http://localhost:3000/api",
      apiKeyPreview: apiKey ? `${apiKey.substring(0, 20)}...` : "MISSING",
      funnelIdPreview: funnelId ? `${funnelId.substring(0, 20)}...` : "MISSING",
    });

    if (!apiKey || !funnelId) {
      console.error(
        "[Aurea] Missing API key or Funnel ID. Tracking disabled.",
        { apiKey: !!apiKey, funnelId: !!funnelId }
      );
      return;
    }

    console.log("[Aurea] Initializing SDK...");
    const sdk = initAurea({
      apiKey,
      funnelId,
      apiUrl: apiUrl || "http://localhost:3000/api",
      debug: true, // Enable debug mode to see events
      autoTrack: {
        pageViews: true,
        forms: true,
        scrollDepth: true,
        clicks: false,
      },
      autoAdvanceStages: true, // Enable automatic stage progression
    });
    
    // Register custom event categories for TTR
    // Users can define their own event names and categories
    sdk.registerEventCategories({
      // VIEWING category - Page content consumption
      // These should only fire once per session
      'hero_viewed': {
        category: 'viewing',
        advanceTo: 'awareness',
        value: 10,
        description: 'User scrolled to hero section',
        trackOnce: true
      },
      'testimonials_viewed': {
        category: 'viewing',
        advanceTo: 'awareness',
        value: 15,
        description: 'User scrolled to testimonials section',
        trackOnce: true
      },
      'stats_viewed': {
        category: 'viewing',
        advanceTo: 'awareness',
        value: 20,
        description: 'User scrolled to Darwinex stats section',
        trackOnce: true
      },
      
      // ENGAGEMENT category - Active interaction with content
      // Video events can happen multiple times (if user replays)
      'video_started': {
        category: 'engagement',
        advanceTo: 'interest',
        value: 30,
        description: 'User started watching sales video',
        trackOnce: false // Can replay video
      },
      'video_25_percent': {
        category: 'engagement',
        advanceTo: 'interest',
        value: 40,
        description: 'User watched 25% of video',
        trackOnce: false // Can replay
      },
      'video_50_percent': {
        category: 'engagement',
        advanceTo: 'interest',
        value: 55,
        description: 'User watched 50% of video',
        trackOnce: false // Can replay
      },
      'video_75_percent': {
        category: 'engagement',
        advanceTo: 'desire',
        value: 75,
        description: 'User watched 75% of video - high intent!',
        trackOnce: false // Can replay
      },
      'video_completed': {
        category: 'engagement',
        advanceTo: 'desire',
        value: 85,
        description: 'User watched entire video',
        trackOnce: false // Can replay
      },
      'video_replayed': {
        category: 'engagement',
        advanceTo: 'desire',
        value: 80,
        description: 'User replayed video - very high intent',
        trackOnce: false // Obviously can happen multiple times
      },
      
      // INTENT category - Purchase consideration signals
      // faq_opened can fire multiple times (user can open different FAQs)
      'faq_opened': {
        category: 'intent',
        advanceTo: 'desire',
        value: 50,
        description: 'User opened FAQ to research',
        trackOnce: false // Allow multiple (different FAQs)
      },
      // faq_multiple_opened should only fire once
      'faq_multiple_opened': {
        category: 'intent',
        advanceTo: 'desire',
        value: 65,
        description: 'User opened 3+ FAQs - researching heavily',
        trackOnce: true // Only track the first time they hit 3+ FAQs
      },
      'pricing_section_viewed': {
        category: 'intent',
        advanceTo: 'desire',
        value: 60,
        description: 'User scrolled to pricing/CTA section',
        trackOnce: true // Section view should only fire once
      },
      'cta_hovered': {
        category: 'intent',
        advanceTo: 'desire',
        value: 70,
        description: 'User hovered over buy button',
        trackOnce: true // First hover is what matters
      },
      'benefits_section_viewed': {
        category: 'intent',
        advanceTo: 'desire',
        value: 55,
        description: 'User read benefits section',
        trackOnce: true
      },
      'about_section_viewed': {
        category: 'intent',
        advanceTo: 'interest',
        value: 40,
        description: 'User scrolled to about section',
        trackOnce: true
      },
      
      // CONVERSION category - Direct purchase actions
      // These should only fire once per session
      'buy_button_clicked': {
        category: 'conversion',
        // Don't auto-advance to checkout - checkoutStarted() does that
        value: 90,
        description: 'User clicked buy button - checkout initiated',
        trackOnce: true // Only count first click
      },
      'whop_checkout_opened': {
        category: 'conversion',
        value: 92,
        description: 'Whop checkout modal opened',
        trackOnce: true
      },
      'payment_info_entered': {
        category: 'conversion',
        value: 95,
        description: 'User entered payment information',
        trackOnce: true
      },
      
      // CUSTOM category - Other valuable signals
      // These are milestones and should only fire once
      'scroll_depth_50': {
        category: 'custom',
        advanceTo: 'interest',
        value: 25,
        description: 'User scrolled 50% of page',
        trackOnce: true
      },
      'scroll_depth_75': {
        category: 'custom',
        advanceTo: 'desire',
        value: 45,
        description: 'User scrolled 75% of page',
        trackOnce: true
      },
      'scroll_depth_25': {
        category: 'custom',
        value: 15,
        description: 'User scrolled 25% of page',
        trackOnce: true
      },
      'scroll_depth_90': {
        category: 'custom',
        value: 50,
        description: 'User scrolled 90% of page',
        trackOnce: true
      },
      'scroll_depth_100': {
        category: 'custom',
        value: 55,
        description: 'User scrolled to bottom of page',
        trackOnce: true
      },
      'time_on_page_30s': {
        category: 'custom',
        advanceTo: 'interest',
        value: 30,
        description: 'User spent 30+ seconds on page',
        trackOnce: true
      },
      'time_on_page_60s': {
        category: 'custom',
        advanceTo: 'interest',
        value: 50,
        description: 'User spent 60+ seconds on page',
        trackOnce: true
      },
      'time_on_page_120s': {
        category: 'custom',
        advanceTo: 'desire',
        value: 70,
        description: 'User spent 2+ minutes on page - very engaged!',
        trackOnce: true
      },
      'time_on_page_180s': {
        category: 'custom',
        value: 80,
        description: 'User spent 3+ minutes on page',
        trackOnce: true
      },
      'time_on_page_300s': {
        category: 'custom',
        value: 90,
        description: 'User spent 5+ minutes on page',
        trackOnce: true
      },
      
      // HIGH_ENGAGEMENT category - Detected high engagement patterns
      'high_engagement_detected': {
        category: 'high_engagement',
        color: 'fuchsia', // Bright/vibrant to stand out
        advanceTo: 'desire',
        value: 85,
        description: 'High engagement detected - rapid user interactions',
        trackOnce: true
      },
      
      // SESSION category - Session lifecycle events
      'session_start': {
        category: 'session',
        color: 'cyan', // Cool neutral color for system events
        value: 0,
        description: 'User session started',
        trackOnce: false // Sessions can start/end multiple times (different days)
      },
      'session_end': {
        category: 'session',
        color: 'cyan',
        value: 0,
        description: 'User session ended',
        trackOnce: false
      },
      
      // PERFORMANCE category - Web vitals and performance metrics
      'web_vital': {
        category: 'performance',
        color: 'yellow', // Warning/monitoring color
        value: 0,
        description: 'Core Web Vitals measurement',
        trackOnce: false // Multiple vitals measured
      },
      
      // SYSTEM category - Auto-tracked events
      'page_view': {
        category: 'viewing',
        value: 5,
        description: 'User viewed a page',
        trackOnce: false // Can view multiple pages
      },
      'scroll_depth': {
        category: 'custom',
        value: 10,
        description: 'User scrolled to specific depth on page',
        trackOnce: false // Multiple scroll events per page
      },
      'funnel_stage_entered': {
        category: 'session',
        color: 'cyan',
        value: 0,
        description: 'User entered new funnel stage',
        trackOnce: false // Can enter multiple stages
      },
      
      // THANK YOU PAGE events
      'checkout_completed': {
        category: 'conversion',
        value: 100,
        description: 'User completed checkout - sale confirmed',
        trackOnce: true
      },
      'thank_you_page_viewed': {
        category: 'viewing',
        value: 5,
        description: 'User viewed thank you page after purchase',
        trackOnce: true
      },
      'check_email_clicked': {
        category: 'engagement',
        value: 10,
        description: 'User clicked to check email on thank you page',
        trackOnce: true
      },
      'back_to_home_clicked': {
        category: 'engagement',
        value: 5,
        description: 'User clicked back to home from thank you page',
        trackOnce: true
      },
      'support_email_clicked': {
        category: 'engagement',
        value: 10,
        description: 'User clicked support email link',
        trackOnce: false // May need support multiple times
      },
      
      // CTA SECTION events
      'cta_section_viewed': {
        category: 'intent',
        advanceTo: 'desire',
        value: 60,
        description: 'User viewed CTA section',
        trackOnce: true
      },
      'discord_clicked': {
        category: 'engagement',
        value: 30,
        description: 'User clicked Discord link',
        trackOnce: true
      },
      
      // OTHER events
      'contact_clicked': {
        category: 'intent',
        value: 40,
        description: 'User clicked contact link',
        trackOnce: true
      },
      'checkout_initiated': {
        category: 'conversion',
        value: 85,
        description: 'User initiated checkout process',
        trackOnce: true
      },
      'checkout_exit': {
        category: 'intent',
        value: 0,
        description: 'User exited checkout without completing',
        trackOnce: false // May exit and retry multiple times
      },
      'debug_test_event': {
        category: 'custom',
        value: 0,
        description: 'Debug test event for SDK testing',
        trackOnce: false
      },
    });
    
    console.log("[Aurea] SDK initialized with custom event categories");
    
    // Make SDK available globally for easy access from components
    if (typeof window !== 'undefined') {
      (window as any).aureaSDK = sdk;
    }
  }, []);

  return null;
}
